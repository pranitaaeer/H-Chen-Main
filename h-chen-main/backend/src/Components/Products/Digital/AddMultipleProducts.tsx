...
//                   onDrop={handleDrop}
//                   className={`drag-drop-zone mt-4 ${isDragging ? "dragging" : ""
//                     }`}
//                   style={{
...
const AddMultipleProducts = () => {
  ...
  // ❌ FIX: removed unnecessary DB count dependency loop
  ...
  const processFile = (file: File) => {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        const parsedProducts: ProductValues[] = csvData.map((row) => ({
          title: row.title || "",
          description: row.description || "",
          category: row.category || "",
          price: Number(row.price) || 0,
  ...
  const handleSaveProducts = async () => {
    ...
    try {
      const res = await fetch("/api/products/create/create-multiple", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(products),
      });
